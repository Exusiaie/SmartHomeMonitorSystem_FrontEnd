...
Login::Login(QWidget *parent, QString username, QString password)
    : m_pParent(parent),
      m_username(username),
      m_password(password),
...
void Login::onSocketConnected(){
    log_info("成功连接到服务器，开始发送用户名。");
    sendUsername();
}

void Login::onSocketReadyRead()
{
    log_info("收到了服务器的返回信息");
    // 将新到达的数据追加到缓冲区
...
{
    log_error(QString("连接错误：%1").arg(m_pSocket->errorString()).toUtf8().data());
...
void Login::sendUsername()
{
    ...
    if (tlv_to_send.length < 1024) {
        ...
        log_info(QString("tlv_to_send (username): %1 | %2 | %3").arg(tlv_to_send.type).arg(tlv_to_send.length). arg(tlv_to_send.data).toUtf8().data());
...
void Login::sendPassword(const std::string& salt)
{
    ...
    if (tlv_to_send.length < 1024){
        ...
        log_info(QString("加密并拼接后的密码: %1").arg(encrypted_password).toUtf8().data());
    } else {
...
void Login::parseAndProcess(wd::Packet packet)
{
    ...
    log_info(QString("接收到服务器数据，类型：%1 长度：%2 盐值：%3").arg(packet.type).arg(packet.length).arg(QString::fromStdString(packet.msg)).toUtf8().data());

    if (!m_waitingPasswordStage) {
        // 第一阶段：验证用户名
        if (packet.type == wd::TaskType::TASK_TYPE_LOGIN_SECTION1_RESP_OK) {
            log_info("收到服务器的OK，开始发送密码。");
